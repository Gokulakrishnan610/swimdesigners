...
interface PassiveWaterWaveProps {
...
const PassiveWaterWave: React.FC<PassiveWaterWaveProps> = ({ 
  children, 
  className = "",
  dropRadius = 20,
  perturbance = 0.03,
  ...
  const handleMouseMove = useCallback((e: MouseEvent, dropRipple?: (x: number, y: number, radius: number, strength: number) => void) => {
  ...
  const handleTouchMove = useCallback((e: TouchEvent, dropRipple?: (x: number, y: number, radius: number, strength: number) => void) => {
  ...
  const handleTouchStart = useCallback((e: TouchEvent, dropRipple?: (x: number, y: number, radius: number, strength: number) => void) => {
    if (!interactive || !containerRef.current) return;
  ...
  useEffect(() => {
    ...
    let currentDropRipple: ((x: number, y: number, radius: number, strength: number) => void) | undefined;

    // Function to update the dropRipple function
    const updateDropRipple = (dropFn: (x: number, y: number, radius: number, strength: number) => void) => {
      currentDropRipple = dropFn;
    };
    ...
    const mouseMoveHandler = (e: MouseEvent) => {
      if (element) {
        ...
        if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
          const dropRipple = (element as any).dropRipple;
          handleMouseMove(e, dropRipple);
        }
      }
    };
    ...
    const touchMoveHandler = (e: TouchEvent) => {
      const dropRipple = (element as any).dropRipple;
      handleTouchMove(e, dropRipple);
    };
    const touchStartHandler = (e: TouchEvent) => {
      const dropRipple = (element as any).dropRipple;
      handleTouchStart(e, dropRipple);
    };
  ...
  return (
    ...
    >
      ...
      >
                 {({ drop: dropRipple }) => {
           // Update the dropRipple function when it becomes available
           React.useEffect(() => {
             if (dropRipple && containerRef.current) {
               // Store the dropRipple function for use in event listeners
               const element = containerRef.current;
               (element as any).dropRipple = dropRipple;
             }
           }, [dropRipple]);
...
export default PassiveWaterWave; 