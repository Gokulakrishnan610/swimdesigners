...
const ProductsPage = () => {
  ...
  // Create product data with dynamic image imports
  const productData = [
    {
          ...
          <div key={cat.category} className="mb-12 sm:mb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6 lg:gap-8">
              {cat.products.map((prod, j) => (
                ...
                >
                  <img 
                    src={imageEntries[prod.imageIndex] ? (imageEntries[prod.imageIndex][1] as any).default : 'https://via.placeholder.com/200x200?text=Product+Image'} 
                    alt={prod.name} 
                    className="h-20 sm:h-24 md:h-28 lg:h-32 object-contain mb-3 sm:mb-4 w-full" 
                    onError={(e) => {
